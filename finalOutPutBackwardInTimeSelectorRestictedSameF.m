...
netDelay=[];
netDelayViso=[];
for jo=1:leno(1,1) %jo=9:leno(1,1)
    jm=jm+1;
...
%         end
        modFil=modFil1 ;
        %--------------
%         modalSel=find(fStBetweeEv(:)>=min(modFil1,modFil2)-veciModFil & fStBetweeEv(:)<=max(modFil1,modFil2)+veciModFil);%
        modalSel=find(fStBetweeEv(:)>=modFil1-1 & fStBetweeEv(:)<=modFil1+2);% modalSel=find(fStBetweeEv(:)==modFil);
