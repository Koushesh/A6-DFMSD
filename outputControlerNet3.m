...
if sepZonOutput~=1
    ...
    dateEvTxt=[dateEv,'St',num2str(minLocSearCohStNum+2),'Zon-',partZon,'.txt'];
    finalDateEvTxt=[dateEv,'St',num2str(minLocSearCohStNum+3),'Zon-',partZon,'.txt'];
elseif sepZonOutput==1
    dateEvTxt=[dateEv,'St',num2str(minLocSearCohStNum+2),'Zon-',num2str(Zon(1,ni)),'.txt'];
    finalDateEvTxt=[dateEv,'St',num2str(minLocSearCohStNum+3),'Zon-',num2str(Zon(1,ni)),'.txt'];
end
...
for zz=0:fix((leno(1,1)-1)/4)-1    %fix((leno(1,1)-8)/4)-1 
    ...
    if max(numStEv)>=minLocSearCohStNum+3
        for kk=1:3
            if kk==2
